...
const Checkout = () => {
  ...
  const handlePayment = async () => {
    try {
      const orderRes = await fetch('https://shopnest-xjik.onrender.com/api/payment/order', {
        method: 'POST',
...
try {
  ...
  console.error("Invalid payment response:", responseText);
      ...
      const options = {
        key: 'rzp_test_TOQBi73kBNDUdd', // Student dummy fallback
        ...
        handler: async function (response) {
         const verifyRes = await fetch('https://shopnest-xjik.onrender.com/api/payment/verify', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(response)
          });
          if (verifyRes.ok) {
            const saveOrderRes = await fetch('https://shopnest-xjik.onrender.com/api/orders', {
              method: 'POST',
              headers: { 
